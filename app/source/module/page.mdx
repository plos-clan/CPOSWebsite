...
```cmake
COMMAND python3 ${CMAKE_SOURCE_DIR}/tools/sign_module.py $<TARGET_FILE:内核模块构建代号> ${CMAKE_BINARY_DIR}/keys/module_signing_priv.pem
```

对于关闭内核的签名校验, 在 `metadata.h` 中有一个 `MODULE_CHECK` 宏用于控制编译流程决定是否启用签名校验功能.
